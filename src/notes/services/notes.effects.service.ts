...
export class NotesEffectsService {
    ...
    @Effect() init$ = this.updates$
        .whenAction('INIT_NOTES').do(()=> {
            console.log("tutaj trwa wykonanie rpc", this.notesDataService.list);
            this.notesDataService.doNext();


        }).delay(1).do(()=> {
            console.log("tutaj powinniśmy doczekać się wykonania rpc, eksperymentując z delay", this.notesDataService.list)
        })
